...
| 7c-triggers | design | P0 | Web renderer Phase 7c-triggers — automatic + after trigger dispatch | 7b | Counter scaffold shipped (`6fe225f`: stateElapsed + idleElapsed advance in tick, reset on applyState). Dispatch deferred — naive `fire-when-elapsed >= delay` diverged from recorded Swift Phase1Card trace; gating semantics need Researcher pre-pass against the live Swift engine. |
| 7c-projectile | queued | P0 | Web renderer Phase 7c-projectile — projectile flight physics | 7b, 7c-triggers | ActiveProjectile state + tick integration (gravity, air resistance, collision with bounds, restitution, friction, stopSpeed, restingFrames). ~250 LoC port from `MotionEngine.swift` projectile section. Required for slingshot release-and-fly. |
