...
| 8 | shipped | P0 | Web renderer Phase 8 — Studio editor cutover behind a flag | 7b, 7c-triggers, 7c-projectile | `b2bcb12` MotionPreview.tsx (React component owning MotionRuntime + MotionCanvasRenderer + RAF loop + pointer event wiring + axis-aligned hitTest + center-origin coord conversion) gated by `VITE_FRAMEZERO_RENDERER=motion` or `?renderer=motion`. tsconfig.json adds matching project refs since vite alias alone doesn't satisfy `tsc -b`. studio-web 23 → 28 tests; runtime 30/30 still green. **P0 v1 chain complete.** Open follow-ups deliberately deferred: viewport-size cutover, onTap → editor selection, 3-5 sample velocity accumulator. |
